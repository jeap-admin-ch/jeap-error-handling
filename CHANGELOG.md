...
### Fixed
- Sanitize Modulith error text for PostgreSQL and tolerate unavailable trace context during ingestion.
...
### Changed
- Resolve restricted-identifier findings in the Modulith integration tests.

## [24.1.0] - 2026-09-14

### Added
- Durable target-service headers are enabled by default on Modulith retry/discard commands using outbox header storage.
- Additive Flyway migration for `deferred_message_header`; existing Kafka resend behavior is unchanged.
...
### Changed
- The topic configuration is validated at startup: the error handling service refuses to start if
  `jeap.errorhandling.topic` and `jeap.errorhandling.deadLetterTopicName` name the same topic, or if
  `jeap.errorhandling.topic` is missing.
...
- **conventional-changelog**: 8.1.1 → 8.1.3 (patch)
...
### Dependencies
- **conventional-changelog**: 8.1.0 → 8.1.1 (patch)
...
- **ts-jest**: 29.1.3 → 29.4.12 (minor)
- **picomatch**: 4.0.4 → 4.0.5 (patch)
- **node-gyp**: 12.4.0 → 13.0.1 (major)
- **jest-preset-angular**: 16.2.0 → 17.0.0 (major)
...
- Update Angular to 21.2.19 and fast-uri to 3.1.5 or higher to fix security vulnerabilities.

## [19.0.0] - 2026-08-02

### Dependencies
- **org.codehaus.mojo:exec-maven-plugin**: 3.0.0 → 3.6.3 (minor)
- **ch.admin.bit.jeap:jeap-spring-boot-parent**: 37.2.0 → 38.0.1 (major)

## [18.8.0] - 2026-07-30

### Added
- New property `jeap.errorhandling.frontend.pams-enabled` (default `true`) to deactivate the PAMS/ePortal
...
### Changed
- Update vulnerable transitive frontend dependencies (@babel/core, fast-uri, immutable).

## [18.7.0] - 2026-07-20

### Changed
- Update to jeap-spring-boot-parent 37.0.0 and use standalone WireMock transitively from the security test starter.
...
- Add configurable and locally persisted default ordering for error groups
...
- Update frontend dependencies to fix security vulnerabilities (Angular to 21.2.17, @babel/core to 7.29.6)
...
- New column `original_trace_context_sampled boolean` on `error`. Needed so the sampling decision captured from the
  ...
  Rows written prior to this version carry `NULL` and are treated as sampled on resend (legacy-compatible default).
- Upgrade some frontend dependencies to fix vulnerabilities
...
- use @Async instead of CompletableFuture.runAsync to prevent classloading issues
- flush to ensure all sends have actually reached the broker and then commit sync
...
- Fix mass delete of errors by using a single transaction and skipping already deleted errors instead of failing the whole request
...
- Updated Angular dependencies to version 21
- Updated Oblique to version 15.1.2
- Replaced `@quadrel-services/qd-auth` with `@quadrel-enterprise-ui/auth`
- Updated Jest to version 30
- Added total item count display to paginator in error list, error group list and error group details list
...
- Corrected ErrorStubs to use different publishers for errorEventService and causingEventService. 

## [15.16.1] - 2026-01-29

### Fixed

- Correct resend information header jeap_eh_target_service, take publisher from errorEventMetadata
...
- Adapt ErrorHandlingIT that ErrorHandlingTargetFilter won't filter out
